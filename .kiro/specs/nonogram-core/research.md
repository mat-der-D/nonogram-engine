...
### 決定: ProbingSolver プロービングの片側矛盾ハンドリング

- **背景**: デザインレビューで指摘された設計ギャップ（2026-03-05）
- **問題**: 要件 5.2「両仮定で共通するセルを確定」は両仮定とも有効な場合のみ定義しており、一方が矛盾した場合のルールが未定義だった
- **採用**: 一方の仮定が `Contradiction` を返した場合、他方の値を即座に強制代入してメイングリッドに適用し `LinePropagator::propagate` を再実行する
- **根拠**: これは CSP の「弧整合性」の原理と同等。片側矛盾は確定的な推論であり、Backtracker への委譲ではなく伝播フェーズで処理すべき
- **トレードオフ**: 実装複雑さがわずかに増加するが、探索効率が向上し Backtracker の呼び出し回数が減少する

### 決定: `validation.rs` モジュール命名

- **背景**: デザインレビューで指摘された `mod validate;` と `pub use validate::validate;` の名前衝突リスク（2026-03-05）
- **問題**: 同一スコープに同名のモジュールと再エクスポート項目が共存すると Rust コンパイラが曖昧な名前解決をする可能性があり、要件 8.4「警告なしコンパイル」に違反するリスクがある
- **採用**: ファイル名を `validation.rs`（モジュール名 `validation`）に変更し、`pub use validation::validate;` で関数のみを公開する
- **根拠**: モジュール名と公開関数名が異なることで Rust の名前解決が明確になる
- **トレードオフ**: ファイル名がやや冗長になるが混乱は完全に排除される
