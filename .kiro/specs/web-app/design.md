...
アプリは単一ページ（SPA）で、画面切り替えではなく**UI状態の遷移**によって表示が変化する。

```mermaid
stateDiagram-v2
    [*] --> WasmLoading : アプリ起動
    WasmLoading --> WasmError : 初期化失敗
    WasmLoading --> Idle : 初期化成功

    Idle --> Idle : 行数・列数入力
    Idle --> ClueMode : モード=ヒント入力（デフォルト）
    Idle --> GridMode : モード=グリッド描画

    ClueMode --> GridMode : モード切り替え
    GridMode --> ClueMode : モード切り替え

    ClueMode --> Solving : 求解ボタン押下
    GridMode --> Solving : 求解ボタン押下

    Solving --> ResultNone : status=none
    Solving --> ResultUnique : status=unique
    Solving --> ResultMultiple : status=multiple
    Solving --> ResultError : status=error

    ResultNone --> ClueMode : 再入力（モード維持）
    ResultNone --> GridMode : 再入力（モード維持）
    ResultUnique --> ClueMode : 再入力
    ResultUnique --> GridMode : 再入力
    ResultMultiple --> ClueMode : 再入力
    ResultMultiple --> GridMode : 再入力
    ResultError --> ClueMode : 再入力
    ResultError --> GridMode : 再入力
...
アプリ全体は縦 1 カラムのシングルページレイアウト。以下のセクションを上から順に配置する。

```
┌─────────────────────────────────────────┐
│  [エラーバナー]  ← WASM 初期化失敗時のみ表示    │
├─────────────────────────────────────────┤
│  ヘッダー: アプリタイトル                       │
├─────────────────────────────────────────┤
│  ImportExportPanel                        │
│  [問題インポート] [問題エクスポート] [テンプレート] │
├─────────────────────────────────────────┤
│  PuzzleSizeInput                          │
│  行数: [__] 列数: [__]  [確定]             │
├─────────────────────────────────────────┤
...
│  入力エリア（モードにより切り替え表示）             │
│                                           │
│  [ClueInputPanel]  または  [GridDrawingPanel] │
│                                           │
├─────────────────────────────────────────┤
│  SolveButton                              │
│  [  解く  ]  ← 求解中はスピナー表示・無効化    │
...
│  ResultPanel（求解後のみ表示）                 │
│  ┌─────────────────────────────────┐   │
│  │ 結果ラベル + SolutionGrid × 1〜2  │   │
│  │ [解答エクスポート]                  │   │
│  └─────────────────────────────────┘   │
└─────────────────────────────────────────┘
```

#### ClueInputPanel レイアウト

行ヒントと列ヒントを 2 ブロックに分けて表示する。

```
行ヒント:
  行 1: [_____________]  ← カンマ/スペース区切り入力
  行 2: [_____________]
  ...

列ヒント:
  列 1: [_______]  列 2: [_______]  ...
```

- バリデーションエラーは入力欄の直下に赤字で表示する
- 空入力（空ヒント）は有効とし、エラー表示しない

#### GridDrawingPanel レイアウト

`rows × cols` のセルグリッドを表示する。各セルは正方形。
...
- 塗りつぶしセル（`true`）: 濃色背景
- 空白セル（`false`）: 淡色背景
- ホバー時: カーソル変更（crosshair）でドラッグ可能であることを示す

グリッド下部にリアルタイム計算されたヒントを表示する。
...
┌──────────────┐   ┌──────────────┐
│  SolutionGrid │   │  SolutionGrid │  ← multiple 時のみ 2 列
│  (解例 1)     │   │  (解例 2)     │
└──────────────┘   └──────────────┘

[解答エクスポート]  ← status=unique/multiple のときのみ有効
```
...
| 状態 | WasmStatus | inputMode | solvePhase | 表示される要素 |
|------|-----------|-----------|-----------|--------------|
| 起動中 | loading | — | idle | ローディングスピナー（全画面または上部） |
| WASM エラー | error | — | idle | エラーバナー、入力UI（求解ボタンは disabled） |
| 初期状態 | ready | clue | idle | PuzzleSizeInput, ModeToggle, ClueInputPanel, SolveButton |
| グリッドモード | ready | grid | idle | PuzzleSizeInput, ModeToggle, GridDrawingPanel, SolveButton |
| 求解中 | ready | any | solving | 入力UI（変更不可）, SolveButton（スピナー・disabled） |
| 解なし | ready | any | done/none | ResultPanel「解が存在しないパズルです」, エクスポートボタン disabled |
| 唯一解 | ready | any | done/unique | ResultPanel「唯一解」ラベル + SolutionGrid × 1, エクスポートボタン enabled |
| 複数解 | ready | any | done/multiple | ResultPanel SolutionGrid × 2 並列, エクスポートボタン enabled |
| 求解エラー | ready | any | done/error | ResultPanel エラーメッセージ, エクスポートボタン disabled |
